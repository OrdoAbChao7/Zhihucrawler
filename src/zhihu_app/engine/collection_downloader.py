...
class CollectionDownloader:
    ...
    def download_items(self, items: list[dict[str, Any]], output_dir: Path, emit) -> int:
        ...
        for item in items:
            ...
            # ⚡ Bolt: Use short-circuit evaluation to prevent unconditionally executing
            # _content_preview(item), which performs expensive regex replacements
            # on large strings. This avoids CPU overhead when url or id is present.
