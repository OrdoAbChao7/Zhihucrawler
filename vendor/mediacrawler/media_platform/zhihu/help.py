...
def _extract_answers_from_html(self, html_content: str, question_id: str, max_answers: int = 50) -> List[Dict]:
    """
    Extract answers from question page HTML content
    
    Args:
        html_content: HTML content of the question page
    ...
    try:
        # Try to extract data from initialData script tag
        selector = Selector(text=html_content)
        js_init_data = selector.xpath("//script[@id='js-initialData']/text()").get(default="")
        ...
        if js_init_data:
            try:
                ...
                answers_by_question = entities.get("answersByQuestion", {})
                question_answers = answers_by_question.get(question_id, [])
                
                if question_answers:
                    for answer_id in question_answers[:max_answers]:
                        answer_data = answers_dict.get(str(answer_id))
                        if answer_data:
                            # Get user info
                            ...
                            answers.append({
                                ...
                                "answer_url": f"{zhihu_constant.ZHIHU_URL}/question/{question_id}/answer/{answer_data.get('id', '')}",
                            })
            except json.JSONDecodeError:
                utils.logger.error("[ZhihuExtractor._extract_answers_from_html] Failed to parse JSON data")
        else:
            # Fallback: try to extract answers from HTML elements
            answer_items = selector.css('.List-item, .QuestionAnswer-content, [data-za-detail-view-path-module="AnswerItem"]')
            ...
            for idx, item in enumerate(answer_items):
                ...
                try:
                    # Extract answer URL and ID
                    answer_link = item.css('a[href*="/answer/"]::attr(href)').get(default="")
                    answer_id = ""
                    if answer_link:
                        import re
                        match = re.search(r'/answer/(\d+)', answer_link)
                        if match:
                            answer_id = match.group(1)
                    
                    # Extract content
                    content_el = item.css('.RichText.ztext.ztext--RichContent, .RichContent')
                    content_html = content_el.get() if content_el else ""
                    
                    # Extract excerpt
                    excerpt_el = item.css('.RichContent-inner')
                    excerpt = excerpt_el.css('::text').get(default="")[:200] if excerpt_el else ""
                    
                    # Extract author
                    author_el = item.css('.AuthorInfo-name, .UserLink-link')
                    author_name = author_el.css('::text').get(default="") if author_el else ""
                    
                    # Extract vote count
                    vote_el = item.css('.VoteButton--up .VoteButton-count, .VoteButton-count')
                    vote_text = vote_el.css('::text').get(default="0") if vote_el else "0"
                    voteup_count = int(vote_text.replace("赞同", "").strip()) if vote_text else 0
                    
                    # Extract time
                    time_el = item.css('.ContentItem-time')
                    time_text = time_el.css('::text').get(default="") if time_el else ""
                    ...
                    answers.append({
                        ...
                        "answer_url": answer_link,
                    })
                except Exception as e:
                    utils.logger.error(f"[ZhihuExtractor._extract_answers_from_html] Error extracting answer: {e}")
                    continue
                    
    except Exception as e:
        utils.logger.error(f"[ZhihuExtractor._extract_answers_from_html] Error: {e}")
    
    return answers

# Add the method to ZhihuExtractor class
ZhihuExtractor._extract_answers_from_html = _extract_answers_from_html
