...
class ZhiHuClient(AbstractApiClient, ProxyRefreshMixin):
    ...
    async def request(self, method, url, **kwargs) -> Union[str, Any]:
        ...
        if response.status_code != 200:
            ...
            if response.status_code == 403:
                # 遇到403，尝试使用浏览器访问获取真实的Cookie
                if self.playwright_page:
                    try:
                        # 对于API请求，先访问用户主页获取会话信息
                        if "api/v4/members" in url:
                            # 提取用户主页URL
                            user_token = url.split("/members/")[1].split("/")[0]
                            user_homepage = f"https://www.zhihu.com/people/{user_token}"
                            utils.logger.info(f"[ZhiHuClient.request] 遇到403，尝试使用浏览器访问用户主页: {user_homepage}")
                            await self.playwright_page.goto(user_homepage, wait_until="domcontentloaded")
                            await asyncio.sleep(5)  # 增加等待时间，模拟用户浏览
                        else:
                            utils.logger.info(f"[ZhiHuClient.request] 遇到403，尝试使用浏览器访问: {url}")
                        ...
                        utils.logger.error(f"[ZhiHuClient.request] 浏览器访问失败: {e}")
                raise ForbiddenError(response.text)
            elif response.status_code == 404:  # Content without comments also returns 404
                return {}
            elif response.status_code in [301, 302, 303, 307, 308]:
                # 处理重定向
                utils.logger.info(f"[ZhiHuClient.request] Redirected to: {response.headers.get('Location')}")
                # 手动跟随重定向
                redirect_url = response.headers.get('Location')
                if redirect_url:
                    return await self.request(method, redirect_url, **kwargs)
    ...
    async def get_question_answers(
        ...
        Args:
            ...
            sort_by: Sort method (default, updated, created, voteup_count)
        ...
        params = {
            "include": "data[*].content,excerpt,created_time,updated_time,voteup_count,comment_count,is_normal,admin_closed_comment,can_comment,comment_permission,review_info,reaction_instruction,is_labeled,label_info;data[*].author.badge[?(type=best_answerer)].topics;data[*].author.vip_info;data[*].author;data[*].question.has_publishing_draft,relationship",
            "offset": offset,
            "limit": limit,
            "sort_by": sort_by,
        }
    ...
    async def get_all_answers_by_question(
        ...
        """
        Get all answers under a specific question with a limit on the number of answers
        Args:
            question_id: Question ID
        ...
        while len(all_contents) < max_answers:
            remaining = max_answers - len(all_contents)
            current_limit = min(limit, remaining)

            res = await self.get_question_answers(question_id, offset, current_limit)
            if not res:
                break
            ...
            contents = self._extractor._extract_content_list(res.get("data", []))
            
            # Filter for answer type only
            answer_contents = [c for c in contents if c.content_type == "answer"]
            
            if not answer_contents:
                break

            # Add question_id to the extracted content
            for content in answer_contents:
                if not content.question_id:
                    content.question_id = question_id

            if callback:
                await callback(answer_contents)

            all_contents.extend(answer_contents)

            if is_end or len(answer_contents) < current_limit:
                break
