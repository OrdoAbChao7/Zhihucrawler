...
class AsyncFileWriter:
    ...
    def _get_file_path(self, file_type: str, item_type: str, user_id: str = None) -> str:
        if user_id:
            base_path = f"data/{self.platform}/{file_type}/{user_id}"
        else:
    ...
    async def write_to_csv(self, item: Dict, item_type: str):
        ...
        file_path = self._get_file_path('csv', item_type, user_id)
    ...
    async def write_single_item_to_json(self, item: Dict, item_type: str):
        user_id = item.get('user_id')
        file_path = self._get_file_path('json', item_type, user_id)
        async with self.lock:
            existing_data = []
