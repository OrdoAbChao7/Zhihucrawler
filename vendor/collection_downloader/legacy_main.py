...
# --- 知乎API交互 ---
def get_answer_count(collection_url, session, headers, params_template):
    """
    ...
    api_collection_id = collection_url.split('/')[-1]
...
def get_member_articles_count(member_url, session, headers):
    """
    通过知乎API获取指定用户的文章总数。
    Args:
        member_url (str): 用户文章页面的URL (如: https://www.zhihu.com/people/user_id/posts)。
        session (requests.Session): 请求会话对象。
        headers (dict): 用于请求的HTTP头。
    Returns:
        int: 用户文章总数，获取失败则返回0。
    """
    member_id = member_url.split('/')[-2]
    api_url = f"https://www.zhihu.com/api/v4/members/{member_id}/articles"
    try:
        ...
        print(f"    通过API获取用户文章总数失败 (get_member_articles_count): {e}。")
        return 0


def get_page_json(collection_api_url_base, session, headers, params):
    """
    ...
    """
    try:
        response = session.get(collection_api_url_base, params=params, headers=headers, timeout=10)
...
def get_member_articles_page_json(member_api_url_base, session, headers, params):
    """
    从知乎用户文章API分页获取文章列表的JSON数据。
    Args:
        member_api_url_base (str): 用户文章API的基础URL。
        session (requests.Session): 请求会话对象。
        headers (dict): 请求HTTP头。
        params (dict): 包含offset和limit的分页参数。
    Returns:
        list: 包含当前页面文章JSON对象的列表，失败则返回空列表。
    """
    try:
        response = session.get(member_api_url_base, params=params, headers=headers, timeout=10)
        response.raise_for_status()
        data = response.json().get('data', [])
        response.close()
        return data
    except Exception as e:
        print(f"    API请求或JSON解析错误 (get_member_articles_page_json): {e}")
    return []
...
# --- 主要处理循环 ---
def download_collection_items(start_offset, total_items, collection_url, params_template, session, headers, collection_md_save_path_abs, global_image_root_path_abs):
    """
    下载指定收藏夹中的所有项目，并将它们保存为Markdown文件。
    在下载前会先判断是否需要跳过该项目（基于Frontmatter的URL和修改时间）。
    """
    current_offset_for_retry = start_offset
    ...
    try:
        for offset_val in range(start_offset, total_items, params_template['limit']):
            ...
            items_on_page = get_page_json(collection_api_url_base, session, headers, current_params)
        ...
        time.sleep(5)
        download_collection_items(current_offset_for_retry, total_items, collection_url, params_template, session, headers, collection_md_save_path_abs, global_image_root_path_abs)
...
def download_member_articles(start_offset, total_items, member_url, params_template, session, headers, md_save_path_abs, global_image_root_path_abs):
    """
    下载指定用户的所有文章，并将它们保存为Markdown文件。
    在下载前会先判断是否需要跳过该项目（基于Frontmatter的URL和修改时间）。
    """
    current_offset_for_retry = start_offset
    member_id = member_url.split('/')[-2]
    member_api_url_base = f"https://www.zhihu.com/api/v4/members/{member_id}/articles"
    try:
        for offset_val in range(start_offset, total_items, params_template['limit']):
            current_offset_for_retry = offset_val
            page_num = int(offset_val / params_template['limit']) + 1
            ...
            print(f'\n正在获取第 {page_num} 页 (偏移量: {offset_val})，来自 {member_url}')
            time.sleep(0.8)

            articles_on_page = get_member_articles_page_json(member_api_url_base, session, headers, current_params)
            if not articles_on_page:
                print(f"    第 {page_num} 页未找到文章，或已到达文章末尾。")
                break

            for idx, article_json_data in enumerate(articles_on_page):
                article_number_overall = offset_val + idx + 1
                print(f"\n正在处理第 {article_number_overall}/{total_items} 篇文章...")

                item_metadata, original_md_body = get_item_metadata_and_content(article_json_data)
                print(f"  标题: {item_metadata['title']}")

                filename_stem_for_md = sanitize_filename(item_metadata['title'])
                if not filename_stem_for_md:
                    filename_stem_for_md = f"未命名文章_{article_number_overall}"

                available_md_filepath_if_not_skipped = get_available_filename(
                    filename_stem_for_md, item_metadata, md_save_path_abs)
                ...
                full_content_before_image_processing = frontmatter_str + original_md_body
                content_with_local_images = process_markdown_images_globally(
                    full_content_before_image_processing, md_save_path_abs, global_image_root_path_abs)
        ...
        print(f"网络错误: {e_req}\n将在5秒后尝试从偏移量 {current_offset_for_retry} 继续...")
        time.sleep(5)
        download_member_articles(current_offset_for_retry, total_items, member_url, params_template, session, headers, md_save_path_abs, global_image_root_path_abs)
    except Exception as e_gen:
        print(f"意外错误: {e_gen}")
        import traceback
        traceback.print_exc()
...
def main():
    ...
    headers = {
        ...
        'x-zse-93': '101_3_3.0',
        'x-zse-96': '2.0_0e2Iny=YopxBiMbsJtdZQqStZveK2BgNkWaL7QamBiOccFd95svXXGJ2YzTaYJWs',
    ...
    # 先访问知乎主页以建立会话
    try:
        session.get('https://www.zhihu.com', headers=headers, timeout=10)
        print("已成功连接到知乎")
    except Exception as e:
        print(f"警告: 连接知乎主页失败: {e}")
    ...
    api_params_template = {'limit': 20, 'offset': 0}
    ...
    for collection_entry in collections_to_process:
        ...
        if not collection_url_str:
            print("跳过此条目: 'url' 字段缺失。")
            continue

        # 判断URL类型
        is_member_articles = '/people/' in collection_url_str and collection_url_str.endswith('/posts')

        # 确定当前条目的Markdown文件保存路径
        if md_save_path_str:
            md_save_path_abs = os.path.abspath(md_save_path_str)
        else:
            if is_member_articles:
                member_id = collection_url_str.split('/')[-2]
                default_dir_name = f"知乎用户文章_{sanitize_filename(member_id)}"
            else:
                collection_id_for_path = collection_url_str.split('/')[-1]
                default_dir_name = f"知乎收藏夹_{sanitize_filename(collection_id_for_path)}"
            md_save_path_abs = os.path.abspath(default_dir_name)

        try:
            os.makedirs(md_save_path_abs, exist_ok=True)
        except Exception as e:
            print(f"错误: 创建Markdown保存目录 '{md_save_path_abs}' 失败: {e}。跳过此条目。")
            continue
        ...
        if is_member_articles:
            try:
                session.get(collection_url_str, headers=html_headers, timeout=10)
                print(f"已访问用户页面: {collection_url_str}")
                api_headers['referer'] = collection_url_str
            except Exception as e:
                print(f"警告: 访问用户页面失败: {e}")

        if is_member_articles:
            print(f"\n\n\n--- 开始处理用户文章: {collection_url_str} ---")
            print(f"Markdown文件将保存至: {md_save_path_abs}")
            try:
                total_articles = get_member_articles_count(collection_url_str, session, api_headers)
                print(f"用户文章总数: {total_articles}")
                if total_articles > 0:
                    download_member_articles(0, total_articles, collection_url_str,
                                           api_params_template, session, api_headers,
                                           md_save_path_abs, global_image_root_path_abs)
                else:
                    print("此用户没有文章或无法确定文章数量。")
            except Exception as e_outer:
                print(f"处理用户文章 {collection_url_str} 时发生严重错误: {e_outer}")
                import traceback
                traceback.print_exc()
        else:
            ...
            print(f"Markdown文件将保存至: {md_save_path_abs}")
            try:
                total_items_in_collection = get_answer_count(collection_url_str, session, headers, api_params_template)
                ...
                if total_items_in_collection > 0:
                    download_collection_items(0, total_items_in_collection, collection_url_str,
                                              api_params_template, session, headers,
                                              md_save_path_abs, global_image_root_path_abs)
                else:
